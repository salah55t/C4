...
RSI_PERIOD: int = 9
RSI_OVERSOLD: int = 30 # لم يتم استخدامها مباشرة كميزة ولكنها جزء من منطق RSI
RSI_OVERBOUGHT: int = 70 # لم يتم استخدامها مباشرة كميزة ولكنها جزء من منطق RSI
EMA_SHORT_PERIOD: int = 8
EMA_LONG_PERIOD: int = 21
VWMA_PERIOD: int = 15
SWING_ORDER: int = 3 # لم يتم استخدامها مباشرة هنا
FIB_LEVELS_TO_CHECK: List[float] = [0.382, 0.5, 0.618] # لم يتم استخدامها مباشرة هنا
FIB_TOLERANCE: float = 0.005 # لم يتم استخدامها مباشرة هنا
LOOKBACK_FOR_SWINGS: int = 50 # لم يتم استخدامها مباشرة هنا
ENTRY_ATR_PERIOD: int = 10
ENTRY_ATR_MULTIPLIER: float = 1.5 # لم يتم استخدامها مباشرة هنا
...
# Constants related to strategy conditions, not directly indicators but influence required data length
RECENT_EMA_CROSS_LOOKBACK: int = 2
MIN_ADX_TREND_STRENGTH: int = 20 # لم يتم استخدامها مباشرة هنا
...
# ---------------------- Candlestick Patterns (نسخ من c4.py) ----------------------
...
def prepare_data_for_ml(df: pd.DataFrame, symbol: str, target_period: int = 5) -> Optional[pd.DataFrame]:
    ...
    يضيف جميع المؤشرات ويزيل الصفوف التي تحتوي على قيم NaN.
    يضيف عمود الهدف 'target' الذي يشير إلى ما إذا كان السعر سيرتفع في الشموع القادمة.
    """
    logger.info(f"ℹ️ [ML Prep] تجهيز البيانات لنموذج التعلم الآلي لـ {symbol} (جميع المؤشرات)...")

    # تحديد الحد الأدنى لطول البيانات المطلوبة لجميع المؤشرات
    min_len_required = max(
        ...
        SUPERTREND_PERIOD, VOLUME_LOOKBACK_CANDLES, RSI_MOMENTUM_LOOKBACK_CANDLES
    ) + target_period + 5 # إضافة 5 كعازل
    ...
    df_calc = df_calc.join(adx_df) # دمج نتائج ADX
    ...
    for col in feature_columns:
        if col not in df_calc.columns:
            logger.warning(f"⚠️ [ML Prep] عمود الميزة المفقود لنموذج ML: {col}. سيتم إضافته كـ NaN.")
    ...
    # التأكد من أن عمود 'close' رقمي قبل التحويل
    ...
    # حساب الهدف المصحح: التحقق مما إذا كان الحد الأقصى للإغلاق المستقبلي أعلى بكثير من الإغلاق الحالي
