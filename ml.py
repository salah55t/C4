...
def calculate_features_v7(df: pd.DataFrame, btc_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if adx_data is not None and not adx_data.empty and adx_col_name in adx_data.columns:
    ...
    if atr_data is not None and not atr_data.empty and atr_col_name in atr_data.columns:
         df_calc[atr_col_name] = atr_data[atr_col_name]
    else:
        logger.warning(f"ATR data not generated by pandas_ta or empty. Filling {atr_col_name} with NaN.")
