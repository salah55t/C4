# ملف c4.py - نسخة V17.3.0 (تحسين الاستراتيجيات)
# --- وصف الإصدار:
# هذا الإصدار يضيف تحسينات على استراتيجيات التداول لزيادة دقة الإشارات وتقليل الإشارات الخاطئة.
# 1.  [جديد] إضافة فلتر حجم التداول (Volume Filter) لتجنب الدخول في الأسواق الخاملة.
# 2.  [محسن] تعزيز استراتيجية BB+Stoch بإضافة فلتر حجم التداول وفلتر زخم MACD.
# 3.  [محسن] تحسين استراتيجية Momentum بإضافة فلتر اتجاه طويل الأجل (EMA 200).
# 4.  [مكتمل] الحفاظ على جميع ميزات التداول وإصلاحات الاستقرار من الإصدار V17.2.0.
...
# --- [جديد] فلتر حجم التداول ---
def check_volume_filter(df: pd.DataFrame, min_volume_percentile: float = 30) -> bool:
    """فلتر حجم التداول لتجنب الإشارات في الأسواق الخاملة"""
    if 'volume' not in df.columns or len(df) < 50: # التأكد من وجود بيانات كافية
        return False
    
    # التأكد من عدم وجود قيم فارغة في آخر 50 شمعة
    if df['volume'].tail(50).isnull().any():
        return False

    current_volume = df['volume'].iloc[-1]
    # استخدام min_periods=20 لضمان حساب المتوسط حتى لو كانت هناك بيانات أقل بقليل
    ...
    # التحقق من أن القيم المحسوبة ليست NaN
...
# --- [محسن] استراتيجية BB+Stoch مع فلاتر إضافية ---
...
# --- [محسن] استراتيجية Momentum مع فلتر الاتجاه طويل الأجل ---
def check_momentum_volatility_strategy(df: pd.DataFrame) -> bool:
    ...
    - Long-term trend filter added.
    """
    needed = {'atr_percent','ema9','ema21','macd','macd_signal','close'}
    if len(df) < 200 or not needed.issubset(df.columns): # زيادة المتطلب إلى 200 لوجود EMA200
        return False

    # --- [جديد] إضافة فلتر اتجاه طويل الأجل ---
    df['ema200'] = df['close'].ewm(span=200, adjust=False).mean()
