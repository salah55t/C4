...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('crypto_bot_v7_with_ichimoku.log', encoding='utf-8'), # <-- تم تحديث اسم الملف
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('CryptoBotV7_With_Ichimoku') # <-- تم تحديث اسم المسجل
    ...
    for attempt in range(retries):
        try:
            ...
            with conn.cursor() as cur:
                # Note: The bot assumes c4i.py and ml.py have already created their tables.
                # It only ensures its own core tables exist.
                cur.execute("""
...
class TradingStrategy:
    ...
    def get_features(self, df_15m: pd.DataFrame, df_4h: pd.DataFrame, btc_df: pd.DataFrame, sr_levels_df: pd.DataFrame, ichimoku_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        try:
            ...
            # 2. Calculate S/R features
            df_featured = calculate_sr_features(df_featured, sr_levels_df)

            # 3. Join and calculate Ichimoku features
            ...
            # 5. Ensure all required columns exist, fill with 0 if not
