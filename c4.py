...
# --- تفعيل استراتيجية الدعم والمقاومة ---
USE_SR_STRATEGY: bool = True
...
class TradingStrategy:
    ...
    def generate_signal(self, df_features: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"ℹ️ [ML Signal] {self.symbol}: Model predicted '{'BUY' if prediction == 1 else 'SELL'}' with {confidence:.2%} confidence.")
            
            return {'prediction': int(prediction), 'confidence': confidence}

        except Exception as e:
            logger.warning(f"⚠️ [ML Signal] {self.symbol}: Error: {e}")
            return None

def generate_signal_from_sr(symbol: str, current_price: float, df_features: pd.DataFrame) -> Optional[Dict[str, Any]]:
    logger.info(f"[{symbol}] 🧠 Checking for S/R-based signal. Price: {current_price:.4f}")
    ...
    supports = sorted([lvl for lvl in sr_levels_df['level_price'].astype(float).tolist() if lvl < current_price], reverse=True)
    resistances = sorted([lvl for lvl in sr_levels_df['level_price'].astype(float).tolist() if lvl > current_price])
    ...
    proximity_threshold = last_atr * 0.5 
    if (current_price - closest_support) > proximity_threshold:
        return None

    target_price = closest_resistance
    stop_loss = closest_support - (last_atr * SL_BUFFER_ATR_PERCENT)

    if target_price <= current_price or stop_loss >= current_price:
        log_rejection(symbol, "S/R Invalid TP/SL", {"TP": target_price, "SL": stop_loss, "Price": current_price})
        return None

    signal = {
        'strategy_name': 'Support_Resistance_v1',
        'tp_sl_data': {
    ...
    logger.info(f"✅ [{symbol}] S/R Signal Generated! Support: {closest_support:.4f}, Resistance: {closest_resistance:.4f}")
    return signal
...
def send_telegram_message(target_chat_id: str, text: str, reply_markup: Optional[Dict] = None) -> bool:
    ...
    try: 
        ...
        logger.error(f"❌ [Telegram] Failed to send message due to a network error: {e}")
        return False
    except Exception as e:
        logger.error(f"❌ [Telegram] An unexpected error occurred while sending message: {e}", exc_info=True)
        return False

# --- ✨ NEW ROBUST FUNCTION / دالة جديدة ومُحصّنة ✨ ---
def send_new_signal_alert(signal_data: Dict[str, Any]):
    symbol = signal_data.get('symbol', 'UNKNOWN')
    logger.info(f"[{symbol}] Building new signal alert message...")
    try:
        ...
        ml_confidence = signal_details.get('ML_Confidence', 'N/A')
        tp_sl_source = signal_details.get('TP_SL_Source', 'N/A')
        ...
        logger.info(f"[{symbol}] Data extracted: Entry={entry}, Target={target}, SL={sl}, Strategy={strategy_name}")
        ...
        logger.info(f"[{symbol}] Metrics calculated: RRR={rrr:.2f}")
        ...
        logger.info(f"[{symbol}] Message string built successfully.")
        ...
        if send_telegram_message(CHAT_ID, message, reply_markup):
            log_and_notify('info', f"New Signal Alert Sent: {symbol} from {strategy_name}", "NEW_SIGNAL")
        else:
            log_and_notify('error', f"Failed to send Telegram alert for new signal: {symbol}", "TELEGRAM_ERROR")

    except Exception as e:
        logger.error(f"❌ CRITICAL: Unhandled exception in send_new_signal_alert for {symbol}: {e}", exc_info=True)
                ...
                for symbol in symbol_batch:
                    try:
                        ...
                        if is_trade_open:
                            ...
                            ml_signal = strategy.generate_signal(df_features)

                            if ml_signal and ml_signal['prediction'] == -1:
                                if ml_signal['confidence'] >= SELL_CONFIDENCE_THRESHOLD and current_price >= float(open_signal['entry_price']) * (1 + MIN_PROFIT_FOR_SELL_CLOSE_PERCENT / 100):
                                    ...
                                    send_telegram_message(CHAT_ID, f"🔴 *إغلاق بإشارة بيع*\n`{symbol}`\nتم إغلاق الصفقة المفتوحة بسعر السوق.")
                                continue

                            elif ml_signal and ml_signal['prediction'] == 1 and ml_signal['confidence'] >= BUY_CONFIDENCE_THRESHOLD:
                                last_atr = df_features.iloc[-1].get('atr', 0)
                                ...
                                if tp_sl_data and float(tp_sl_data['target_price']) > float(open_signal['target_price']):
                                    ...
                                    if update_signal_target_in_db(open_signal['id'], new_tp, new_sl):
                                        open_signals_cache[symbol].update({'target_price': new_tp, 'stop_loss': new_sl})
                                        send_telegram_message(CHAT_ID, f"🔼 *تحديث الهدف*\n`{symbol}`\n*الهدف الجديد:* `${new_tp:,.8g}`\n*الوقف الجديد:* `${new_sl:,.8g}`")
                                continue
                        
                        elif not is_trade_open and slots_available > 0:
                            potential_signal = None
                            
                            ml_signal = strategy.generate_signal(df_features)
                            if ml_signal and ml_signal['prediction'] == 1 and ml_signal['confidence'] >= BUY_CONFIDENCE_THRESHOLD:
                                last_atr = df_features.iloc[-1].get('atr', 0)
                                tp_sl_data = calculate_tp_sl(symbol, current_price, last_atr)
                                if tp_sl_data:
                                    potential_signal = {
                                        'strategy_name': BASE_ML_MODEL_NAME,
                                        'signal_details': {'ML_Confidence': f"{ml_signal['confidence']:.2%}", 'TP_SL_Source': tp_sl_data['source']},
                                        **tp_sl_data
                                    }

                            if not potential_signal and USE_SR_STRATEGY:
                                sr_signal = generate_signal_from_sr(symbol, current_price, df_features)
                                if sr_signal:
                                    potential_signal = {
                                        'strategy_name': sr_signal['strategy_name'],
                                        'signal_details': {'TP_SL_Source': sr_signal['tp_sl_data']['source']},
                                        **sr_signal['tp_sl_data']
                                    }
                            ...
                            if potential_signal:
                                ...
                                if USE_RRR_FILTER:
                                    tp, sl = float(new_signal['target_price']), float(new_signal['stop_loss'])
                                ...
                                logger.info(f"✅ [{symbol}] Signal from '{new_signal['strategy_name']}' passed all filters. Saving...")
                                saved_signal = insert_signal_into_db(new_signal)
                                
                                # --- ✨ NEW LOGGING / تسجيل جديد ✨ ---
                                if saved_signal:
                                    with signal_cache_lock: open_signals_cache[saved_signal['symbol']] = saved_signal
                                    
                                    logger.info(f"[{symbol}] Signal saved. PREPARING TO SEND ALERT.")
                                    send_new_signal_alert(saved_signal)
                                    logger.info(f"[{symbol}] Alert function called. Continuing loop.")
