...
FILTER_PROFILES: Dict[str, Dict[str, Any]] = {
    "STRONG_UPTREND": {
        ...
        "strategy": "MOMENTUM",
        "filters": {
            "adx": 30.0, "rel_vol": 0.5, "rsi_range": (55, 95), "roc": 0.1,
            "slope": 0.01, "min_rrr": 1.5, "min_volatility_pct": 0.40,
            "min_btc_correlation": 0.5, "min_bid_ask_ratio": 1.2
        }
    },
    "UPTREND": {
...
def fetch_historical_data(symbol: str, interval: str, days: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        logger.error(f"❌ [Data] Binance API Error for {symbol}: {e}")
        # If we are banned, it's better to wait for a bit before continuing
        if e.code == -1003:
        ...
        try:
            ...
            for symbol in symbols_to_process:
                ...
                try:
                    ...
                    # --- FIX START ---
                    # Added a delay to prevent API rate limit errors (IP ban).
                    # This slows down the scan but ensures the bot can run continuously.
                    time.sleep(0.75) 
                    # --- FIX END ---
