...
def find_bullish_reversal_signal(df_15m: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if rsi_confirms and volume_confirms:
            logger.info(f"✅ [REVERSAL SIGNAL] Bullish reversal detected for {df_15m.name} (EMA Cross + RSI + Volume).")
            return { "signal_type": "REVERSAL", "reason": "EMA Cross, RSI & Volume Confirmation" }

        return None
    except Exception as e:
        logger.error(f"❌ [{df_15m.name if hasattr(df_15m, 'name') else 'Unknown'}] Error in find_bullish_reversal_signal: {e}")
        return None
            ...
            for symbol in symbols_to_process:
                ...
                try:
                    ...
                    if not ml_signal or ml_signal['confidence'] < BUY_CONFIDENCE_THRESHOLD:
                        if technical_signal['signal_type'] == "REVERSAL":
                    ...
                    if processed_count % SYMBOL_PROCESSING_BATCH_SIZE == 0:
                        logger.info("🗑️ Processed batch. Running memory cleanup...")
                        ml_models_cache.clear(); gc.collect()
                        time.sleep(2)
