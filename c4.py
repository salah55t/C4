...
# --- تعديل: إضافة تتبع (logging) مفصل لزيادة الشفافية ---
import time
import os
import json
import logging
...
def log_rejection(symbol: str, reason_key: str, details: Optional[Dict] = None):
    ...
    # NEW LOG: رسالة رفض أوضح
    log_message = f"🚫 [REJECTED] {symbol} | السبب: {reason_ar} | تفاصيل: {details or '{}'}"
...
def get_exchange_info_map() -> None:
    ...
    try:
        logger.info("⏳ [Exchange Info] جارٍ تحميل قواعد التداول من المنصة...")
...
def fetch_historical_data(symbol: str, interval: str, days: int) -> Optional[pd.DataFrame]:
    ...
    try:
        # NEW LOG: تتبع عملية جلب البيانات
        logger.info(f"⏳ [DATA] جارٍ جلب البيانات التاريخية لـ {symbol} (فريم {interval}, {days} يوم)...")
        start_dt = datetime.now(timezone.utc) - timedelta(days=days)
        start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
        klines = client.get_historical_klines(symbol, interval, start_str)
        if not klines: 
            logger.warning(f"⚠️ [DATA] لا توجد بيانات تاريخية لـ {symbol}.")
            return None
        ...
        # NEW LOG: تأكيد نجاح العملية
        logger.info(f"✅ [DATA] تم جلب ومعالجة {len(df)} شمعة لـ {symbol}.")
        return df.dropna()
    except Exception as e:
...
def load_open_signals_to_cache():
    ...
    try:
        logger.info("⏳ [CACHE] جارٍ تحميل الصفقات المفتوحة إلى الذاكرة...")
        with conn.cursor() as cur:
            ...
            logger.info(f"✅ [CACHE] تم تحميل {len(open_signals)} صفقة مفتوحة أو مغلقة جزئيًا.")
    except Exception as e:
...
def check_ema_stoch_momentum_strategy(df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        if ema_crossover and stochastic_momentum:
            # NEW LOG: تأكيد إشارة الاستراتيجية
            logger.info(f"💡 [STRATEGY] {df.name} - إشارة شراء من استراتيجية EMA/Stoch Momentum.")
            return True
        return False
    ...
    try:
        ...
        if stoch_k_cross_above_d and stoch_below_15 and price_at_lower_band:
            ...
            for name, found in patterns.items():
                if found:
                    # NEW LOG: تأكيد إشارة الاستراتيجية
                    logger.info(f"💡 [STRATEGY] {df.name} - إشارة شراء من استراتيجية BB/Stoch Reversal (نموذج: {name}).")
                    return True, name
        return False, ""
    except Exception: return False, ""
...
def analyze_order_book(symbol: str, entry_price: float) -> Optional[Dict[str, Any]]:
    if not client: return None
    try:
        logger.info(f"🔬 [FILTER] {symbol} - تحليل دفتر الطلبات...")
...
def passes_order_book_check(symbol: str, order_book_analysis: Dict) -> bool:
    if not order_book_analysis.get("passes_strength_check", False):
        ...
        return False
    logger.info(f"✅ [FILTER] {symbol} - فلتر دفتر الطلبات: ناجح (Ratio: {order_book_analysis.get('strength_ratio', 0):.2f}).")
    return True
...
def calculate_dynamic_tp_sl(symbol: str, entry_price: float, df: pd.DataFrame, volatility_regime: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        logger.info(f"✅ [TP/SL] {symbol} - تم حساب الأهداف بنجاح (SL: {stop_loss_price:.4f}, TP1: {partial_tp_price:.4f}).")
        return {
...
def calculate_position_size(symbol: str, entry_price: float, stop_loss_price: float) -> Optional[Decimal]:
    if not client: return None
    try:
        logger.info(f"📐 [SIZE] {symbol} - حساب حجم الصفقة...")
        ...
        logger.info(f"✅ [SIZE] {symbol} - الحجم المحسوب: {adjusted_quantity}.")
...
def place_order(symbol: str, side: str, quantity: Decimal, order_type: str = Client.ORDER_TYPE_MARKET) -> Optional[Dict]:
    ...
    try:
        ...
        logger.info(f"✅ [ORDER] {symbol} - تم تنفيذ الأمر بنجاح (ID: {order['orderId']}).")
        return order
    except Exception as e:
...
def close_signal(signal_id: int, closing_price: float, reason: str) -> bool:
    with signal_cache_lock:
        ...
        if signal_to_close.get('is_real_trade'):
            ...
            if quantity_to_sell_dec > 0:
                logger.info(f"💰 [CLOSING] {symbol_to_close} - إرسال أمر إغلاق للسوق...")
                sell_order = place_order(symbol_to_close, Client.SIDE_SELL, quantity_to_sell_dec)
                if not sell_order: 
                    logger.error(f"❌ [CLOSING] {symbol_to_close} - فشل أمر الإغلاق من المنصة.")
                    return False
    ...
    try:
        ...
        logger.info(f"{emoji} [CLOSED] {symbol_to_close} - السبب: {reason}, الربح: {profit_percentage:.2f}%.")
...
def insert_signal_into_db(signal_data: Dict) -> Optional[Dict]:
    ...
    try:
        # NEW LOG: تأكيد حفظ الإشارة
        logger.info(f"💾 [{signal_data['symbol']}] جارٍ حفظ الإشارة الجديدة في قاعدة البيانات...")
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(f"✅ [{signal_data['symbol']}] تم حفظ الإشارة بنجاح (ID: {saved_signal['id']}).")
...
def toggle_trading_status():
    ...
    with trading_status_lock:
        ...
        logger.info(f"🚨🚨🚨 تم تغيير حالة التداول الحقيقي إلى: {status_msg} 🚨🚨🚨")
...
def execute_partial_close(signal: Dict) -> bool:
    ...
    try:
        ...
        logger.info(f"✅ [PARTIAL TP] {symbol} - تم البيع الجزئي ونقل الـ SL إلى نقطة الدخول ({new_stop_loss}).")
    ...
    while True:
        try:
            ...
            for signal in signals_to_check:
                ...
                # NEW LOG: تتبع الصفقات المفتوحة
                logger.debug(f"[MANAGE] Checking {signal['symbol']} at {current_price:.4f} | SL: {signal['stop_loss']:.4f}, TP1: {signal.get('partial_tp_price', 0):.4f}, TP2: {signal['target_price']:.4f}")
...
def main_loop_enhanced():
    ...
    if not validated_symbols_to_scan:
        logger.critical("❌ [Main Loop] قائمة العملات للمسح فارغة. يتوقف البوت.")
    ...
    while True:
        try:
            logger.info(f"🔄 [Main Loop] بدء دورة مسح جديدة لـ {len(validated_symbols_to_scan)} عملة...")
            ...
            for symbol in symbols_to_process:
                try:
                    with signal_cache_lock:
                        if symbol in open_signals_cache:
                            logger.debug(f"[SKIP] {symbol} لديه صفقة مفتوحة بالفعل.")
                            continue
                        if len(open_signals_cache) >= MAX_OPEN_TRADES:
                            logger.info(f"[SKIP] تم الوصول للحد الأقصى للصفقات المفتوحة ({MAX_OPEN_TRADES}).")
                            break 
                    
                    logger.info(f"🔎 [SCAN] فحص العملة: {symbol}")
                    ...
                    if not strategy_signal_found: 
                        logger.debug(f"[SCAN] {symbol} - لا توجد إشارة من الاستراتيجيات.")
                        continue
                    ...
                    if is_enabled:
                        ...
                        logger.info(f"💡 [SIGNAL] {symbol} - تم العثور على إشارة ولكن التداول الحقيقي معطل.")
