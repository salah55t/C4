...
# ---------------------- إعداد نظام التسجيل (Logging) - V8 ----------------------
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('crypto_bot_v15_momentum.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('CryptoBotV15_Momentum')
...
# ---------------------- إعداد الثوابت والمتغيرات العامة - V8 ----------------------
BASE_ML_MODEL_NAME: str = 'LightGBM_Scalping_V8_With_Momentum'
# ✨ [تعديل] تحديد مجلد النماذج
MODEL_FOLDER: str = 'V8'
...
# --- إعدادات الفلاتر المحسّنة (تم الحفاظ عليها) ---
...
# ---------------------- دوال HTML للوحة التحكم (V8) ----------------------
def get_dashboard_html_v8():
    """
    لوحة تحكم V8 متوافقة مع النموذج الجديد.
    """
    return """
<!DOCTYPE html>
...
<body class="p-4 md:p-6">
    <div class="container mx-auto max-w-7xl">
        <header class="mb-6 flex flex-wrap justify-between items-center gap-4">
            <h1 class="text-3xl md:text-4xl font-black text-white">لوحة تحكم التداول (V8 - Momentum)</h1>
...
# ---------------------- دوال حساب الميزات وتحديد الاتجاه (V8) ----------------------
def calculate_features(df: pd.DataFrame, btc_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    """
    تم تحديث الدالة لتشمل مؤشرات V8 الجديدة.
    """
    df_calc = df.copy()
    ...
    if btc_df is not None and not btc_df.empty:
        ...
        df_calc['btc_correlation'] = merged_df['close'].pct_change().rolling(window=BTC_CORR_PERIOD).corr(merged_df['btc_returns'])
    else:
        df_calc['btc_correlation'] = 0.0

    # --- إضافة مؤشرات V8 الجديدة (Momentum, Velocity, Slope) ---
...
def get_trend_for_timeframe(df: Optional[pd.DataFrame]) -> Dict[str, Any]:
    ...
    try:
        ...
        high_close = (temp_df['high'] - temp_df['close'].shift()).abs()
        low_close = (temp_df['low'] - temp_df['close'].shift()).abs()
        tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
        atr = tr.ewm(span=ATR_PERIOD, adjust=False).mean()
        up_move = temp_df['high'].diff(); down_move = -temp_df['low'].diff()
        plus_dm = pd.Series(np.where((up_move > down_move) & (up_move > 0), up_move, 0.0), index=temp_df.index)
        minus_dm = pd.Series(np.where((down_move > up_move) & (down_move > 0), down_move, 0.0), index=temp_df.index)
        ...
        dx = 100 * (abs(plus_di - minus_di) / (plus_di + minus_di).replace(0, 1e-9))
        adx_val = dx.ewm(span=ADX_PERIOD, adjust=False).mean().iloc[-1]
        delta = temp_df['close'].diff()
        gain = delta.clip(lower=0).ewm(com=RSI_PERIOD - 1, adjust=False).mean()
        loss = -delta.clip(upper=0).ewm(com=RSI_PERIOD - 1, adjust=False).mean()
        rsi_val = (100 - (100 / (1 + (gain / loss.replace(0, 1e-9))))).iloc[-1]
        
        ema_fast = temp_df['close'].ewm(span=12, adjust=False).mean().iloc[-1]
        ema_slow = temp_df['close'].ewm(span=26, adjust=False).mean().iloc[-1]
        trend = "Ranging"
        if adx_val > 20:
            if ema_fast > ema_slow and rsi_val > 50: trend = "Uptrend"
            elif ema_fast < ema_slow and rsi_val < 50: trend = "Downtrend"
        return {"trend": trend, "rsi": float(rsi_val), "adx": float(adx_val)}
    except Exception as e:
...
# ✨ [تعديل] دالة جديدة لتحميل النموذج من الملفات المحلية
def load_ml_model_bundle_from_folder(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    if not os.path.exists(model_path):
        logger.debug(f"⚠️ [ML Load] Model file not found at '{model_path}'.")
        return None
    try:
        ...
        if 'model' in model_bundle and 'scaler' in model_bundle and 'feature_names' in model_bundle:
            ...
            logger.info(f"✅ [ML Load] Loaded model '{model_name}' successfully from file.")
            return model_bundle
        else:
            logger.error(f"❌ [ML Load] Model bundle at '{model_path}' is incomplete.")
            return None
    except Exception as e:
        logger.error(f"❌ [ML Load] Error loading model for {symbol} from file: {e}", exc_info=True)
        return None

class TradingStrategy:
    def __init__(self, symbol: str):
        self.symbol = symbol
        # ✨ [تعديل] استخدام الدالة الجديدة لتحميل النموذج من الملفات
    ...
    def get_features(self, df_15m: pd.DataFrame, df_4h: pd.DataFrame, btc_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            mtf_cols_to_join = [col for col in ['rsi_4h', 'price_vs_ema50_4h'] if col in df_4h_features.columns]
            df_featured = df_featured.join(df_4h_features[mtf_cols_to_join], how='outer')
    ...
    def generate_signal(self, df_features: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"ℹ️ [{self.symbol}] Model predicted '{'BUY' if prediction == 1 else ('SELL' if prediction == -1 else 'HOLD')}' with {confidence:.2%} confidence.")
...
def send_new_signal_alert(signal_data: Dict[str, Any]):
    ...
    message = (f"💡 *توصية تداول جديدة (V8 Momentum)* 💡\n\n*العملة:* `{symbol}`\n*حالة السوق:* `{market_regime}`\n\n"
               f"*الدخول:* `{entry:,.8g}`\n*الهدف:* `{target:,.8g}`\n*وقف الخسارة:* `{sl:,.8g}`\n\n"
...
if __name__ == "__main__":
    logger.info(f"🚀 Starting Trading Bot - V8 Momentum Model (File Loading)...")
