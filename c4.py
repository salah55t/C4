...
class SignalGenerator:
    ...
    def _check_strategies(self, df: pd.DataFrame, market_state: MarketState, symbol: str) -> Optional[Tuple[str, int]]:
        ...
        strategy_config = {
            ...
            'Momentum_Strategy': trading_config.USE_MOMENTUM_VOLATILITY_STRATEGY,
        }
...
def get_open_signals():
    try:
        signals = db_manager.execute_query(
            "SELECT * FROM signals WHERE status IN ('open', 'updated') ORDER BY id DESC;",
            fetch=True
        )
        return jsonify({"signals": [dict(row) for row in signals]})
    except Exception as e:
