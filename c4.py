...
def execute_db_query(query, params=None, fetch=None):
    ...
    try:
        ...
        logger.error(f"❌ [DB Query] فشل تنفيذ الاستعلام: {query[:100]}... - {e}")
...
def update_btc_data_cache():
    ...
    else: logger.warning("⚠️ [BTC Data] فشل تحديث بيانات البيتكوين.")
...
def close_signal(signal: Dict, status: str, closing_price: float, closed_by: str):
    ...
    log_and_notify('info', msg.replace('*', ''), "TRADE_CLOSE", send_tg=True)
...
def handle_ticker_message(msg):
    ...
    symbol, price = msg['s'], float(msg['c'])
    with prices_lock: current_prices[symbol] = price
    with signal_cache_lock: signal = open_signals_cache.get(symbol)
...
def insert_signal_into_db(signal: Dict):
    ...
    if new_signal_record:
        ...
        log_and_notify('info', msg.replace('*', ''), "NEW_TRADE", send_tg=True)
...
@app.route('/')
def home(): return "Crypto Trading Bot V5 API is running.", 200
