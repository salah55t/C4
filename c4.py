...
# ✨ تم تغيير المستوى إلى DEBUG لعرض رسائل تتبع الأخطاء التفصيلية.
# ✨ يمكنك إعادته إلى logging.INFO بعد التأكد من أن كل شيء يعمل بشكل صحيح.
logging.basicConfig(
    level=logging.INFO,
...
def initiate_signal_closure(symbol: str, signal_to_close: Dict, status: str, closing_price: float):
    """
    Handles the logic of marking a signal for closure and starting the closing thread.
    This prevents code duplication and centralizes the closure initiation logic.
    """
    ...
    with signal_cache_lock:
        # Pop from the main cache to prevent it from being processed again
    ...
    if signal_data_for_thread:
        ...
        # The actual closing (DB update, notification) is done in a separate thread
        ...
        # Since it's not in the cache, we can remove it from the pending set to be safe
        with closure_lock:
...
def trade_monitoring_loop():
    """
    ✨ [آلية مُحسَّنة] - حلقة المراقبة الرئيسية للصفقات المفتوحة.
    - تستخدم تسجيلًا تفصيليًا (DEBUG level) لتتبع كل خطوة.
    - تتحقق من صحة البيانات بشكل صارم وتُسجل تحذيرًا عند وجود خطأ.
    - تم إعادة هيكلتها لتكون أكثر وضوحًا وموثوقية.
    """
    logger.info("✅ [Trade Monitor] خيط مراقبة الصفقات المُحسَّن بدأ بالعمل.")
    while True:
        try:
            with signal_cache_lock:
            ...
            # --- قسم المراقبة المُحسَّن مع تسجيل مُفصَّل ---
            for symbol, signal in signals_to_check.items():
                ...
                # تسجيل كل عملية تحقق لتسهيل التصحيح
                logger.debug(f"[MONITOR CHECK] SignalID: {signal_id}, Symbol: {symbol}, Price: {price}, TP: {target_price}, SL: {stop_loss_price}")

                if not price:
                    logger.debug(f"[MONITOR SKIP] No price for {symbol} in Redis.")
                    continue
                
                with closure_lock:
                    if signal_id in signals_pending_closure:
                        logger.debug(f"[MONITOR SKIP] Signal {signal_id} for {symbol} is already pending closure.")
                        continue

                # التحقق من صحة البيانات مع تسجيل خطأ واضح
                if not all(isinstance(p, (int, float)) and p > 0 for p in [price, target_price, stop_loss_price]):
                    logger.warning(
                        f"⚠️ [MONITOR SKIP] Invalid/Incomplete data for {symbol} (ID: {signal_id}). "
                        f"Price: {price} (Type: {type(price)}), "
                        f"TP: {target_price} (Type: {type(target_price)}), "
                        f"SL: {stop_loss_price} (Type: {type(stop_loss_price)})"
                    )
                    continue
                ...
                if should_close:
                    logger.info(f"✅ [MONITOR TRIGGER] Condition '{status_to_set}' met for {symbol} at price {price}. Initiating close for signal ID {signal_id}.")
                    initiate_signal_closure(symbol, signal, status_to_set, price)

            time.sleep(0.2) # زيادة طفيفة في فترة الانتظار لتقليل الحمل على وحدة المعالجة المركزية
