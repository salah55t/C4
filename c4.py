...
# ---------------------- إعداد نظام التسجيل (Logging) - V27.0 (Order Book Analysis) ----------------------
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('crypto_bot_v27_arabic_logs.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('CryptoBotV27')
...
FILTER_PROFILES: Dict[str, Dict[str, Any]] = {
    "STRONG_UPTREND": {
        ...
        "strategy": "MOMENTUM",
        "filters": { "adx": 30.0, "rel_vol": 1.0, "rsi_range": (55, 95), "roc": 0.2, "slope": 0.0, "min_rrr": 1.2, "min_volatility_pct": 0.25, "min_btc_correlation": -0.1, "min_bid_ask_ratio": 1.1 }
    },
    "UPTREND": {
        "description": "اتجاه صاعد",
        "strategy": "MOMENTUM",
        "filters": { "adx": 24.0, "rel_vol": 0.4, "rsi_range": (50, 90), "roc": 0.05, "slope": 0.12, "min_rrr": 1.3, "min_volatility_pct": 0.20, "min_btc_correlation": 0.0, "min_bid_ask_ratio": 1.2 }
    },
    "RANGING": {
        "description": "اتجاه عرضي (بحث عن زخم قصير)",
        "strategy": "MOMENTUM",
        "filters": { "adx": 15.0, "rel_vol": 0.2, "rsi_range": (40, 70), "roc": 0.05, "slope": 0.0, "min_rrr": 1.4, "min_volatility_pct": 0.25, "min_btc_correlation": -0.2, "min_bid_ask_ratio": 1.3 }
    },
    "DOWNTREND": {
    ...
    "WEEKEND": {
        ...
        "strategy": "MOMENTUM",
        "filters": { "adx": 17.0, "rel_vol": 0.2, "rsi_range": (30, 70), "roc": 0.1, "slope": 0.0, "min_rrr": 1.5, "min_volatility_pct": 0.25, "min_btc_correlation": -0.4, "min_bid_ask_ratio": 1.4 }
    }
}
...
# --- تعديل: ثوابت جديدة لتحليل دفتر الطلبات ---
ORDER_BOOK_DEPTH_LIMIT: int = 100
ORDER_BOOK_WALL_MULTIPLIER: float = 10.0 # يعتبر الأمر "جدار" إذا كان حجمه أكبر 10 مرات من المتوسط
ORDER_BOOK_ANALYSIS_RANGE_PCT: float = 0.02 # تحليل الأوامر ضمن نطاق 2% من أفضل سعر
...
<head>
    ...
    <title>لوحة تحكم التداول V27.0</title>
    ...
    <div class="container mx-auto max-w-screen-2xl">
        <header class="mb-6 flex flex-wrap justify-between items-center gap-4">
            <h1 class="text-2xl md:text-3xl font-extrabold text-white">
                ...
                <span class="text-text-secondary font-medium">V27.0</span>
            </h1>
...
# --- تعديل: دالة جديدة لجلب وتحليل دفتر الطلبات ---
def analyze_order_book(symbol: str, entry_price: float) -> Optional[Dict[str, Any]]:
    """
    يجلب دفتر الطلبات ويحلله لحساب اختلال التوازن وكشف الجدران.
    """
    if not client: return None
    try:
        ...
        # حساب إجمالي حجم التداول ضمن نطاق معين من السعر الحالي
        ...
        # البحث عن جدران بيع ضمن نطاق 5% أعلى من سعر الدخول
...
# --- تعديل: دالة جديدة للتحقق من فلتر دفتر الطلبات ---
def passes_order_book_check(symbol: str, order_book_analysis: Dict, profile: Dict) -> bool:
    """
    يتحقق مما إذا كانت نتائج تحليل دفتر الطلبات تفي بالمتطلبات المحددة في ملف الفلتر.
    """
    filters = profile.get("filters", {})
    min_ratio = filters.get('min_bid_ask_ratio', 1.0) # الحد الأدنى لنسبة الشراء/البيع
    
    # التحقق من وجود جدار بيع ضخم
    ...
    # التحقق من نسبة الشراء إلى البيع
...
def send_new_signal_alert(signal_data: Dict[str, Any]):
    ...
    bid_ask_ratio_display = f"{signal_details.get('Bid_Ask_Ratio', 0):.2f}"
    ...
    message = (f"{title}\n\n*العملة:* `{symbol}`\n*ملف الفلتر:* `{filter_profile_name}`\n{trade_type_msg}\n"
               ...
               f"*نسبة الشراء/البيع:* `{bid_ask_ratio_display}`")
        ...
        try:
            ...
            for symbol in symbols_to_process:
                ...
                try:
                    ...
                    # --- تعديل: إضافة خطوة التحقق من دفتر الطلبات ---
                    ...
                    if not order_book_analysis or not passes_order_book_check(symbol, order_book_analysis, filter_profile):
                        continue # تم الرفض بواسطة فلتر دفتر الطلبات
                    ...
                    new_signal = {
                        ...
                        'signal_details': {
                            ...
                            # --- تعديل: إضافة بيانات دفتر الطلبات إلى تفاصيل الإشارة ---
                            'Bid_Ask_Ratio': order_book_analysis.get('bid_ask_ratio', 0)
                        },
...
if __name__ == "__main__":
    logger.info("🚀 LAUNCHING TRADING BOT & DASHBOARD (V27.0 - Order Book Analysis) 🚀")
