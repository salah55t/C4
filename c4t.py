...
# --- !!! جديد: فلاتر الاستراتيجية !!! ---
USE_RSI_FILTER: bool = True
RSI_LOWER_THRESHOLD: float = 40.0
RSI_UPPER_THRESHOLD: float = 69.0
USE_MACD_CROSS_FILTER: bool = True # *** إضافة جديدة: تفعيل فلتر تقاطع MACD ***
...
def calculate_features(df: pd.DataFrame) -> pd.DataFrame:
    df_calc = df.copy()
    # *** تعديل: تحديث معلمات المؤشرات لتطابق البوت الرئيسي ***
...
def run_backtest_for_symbol(symbol: str, data: pd.DataFrame, model_bundle: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(df_featured)):
        ...
        # --- *** تعديل: تطبيق فلتر MACD و RSI *** ---
        passes_macd_filter = True
        if USE_MACD_CROSS_FILTER:
            if current_candle.get('macd_cross') != 1:
                passes_macd_filter = False
        ...
        if not in_trade and passes_macd_filter and passes_rsi_filter and current_candle['prediction'] >= MODEL_PREDICTION_THRESHOLD:
            in_trade = True
            entry_price = current_candle['close']
            atr_value = current_candle['atr']
...
def generate_report(all_trades: List[Dict[str, Any]]):
    ...
    filters_used = []
    if USE_RSI_FILTER:
        filters_used.append(f"RSI ({RSI_LOWER_THRESHOLD}-{RSI_UPPER_THRESHOLD})")
    if USE_MACD_CROSS_FILTER:
        filters_used.append("MACD Cross")
    
    if filters_used:
        report_header += " (with filters: " + ", ".join(filters_used) + ")"
